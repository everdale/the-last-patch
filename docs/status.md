...
- **Next step:** a **POC** — see [poc-brief.md](poc-brief.md). **Built (2nd pass):** [demo/poc.html](../demo/poc.html). The first pass (Penultima/meteor) was red-penned for thin gameplay (a two-option solve), a tired scenario, and a tool-mode/modal input that could trap you. The rework moves to a **fresh world — *BOGWILD*, an early-access game stuck in eternal Early Access** (placeholder jank is *diegetic*, so "no committed art" is the joke) and proves the loop through a **real multi-step puzzle**: classic adventure input (**left-click interact, right-click look**), the **Spanner is now an inventory item** (use-on-world + combine), and you trace the gates by inspecting, gather/combine items, and make two debugger-only patches (`price: $NaN → $0`, bench `[WIP] → READY`) to craft the first Torch and end the infinite tutorial. Bill: BUG-256 → BUG-257 (everyone graduates into an unbuilt Zone 2); AUTOPATCH cameos to paper over it. Candidate; awaits the red pen.
