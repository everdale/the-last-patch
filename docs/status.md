...
- **Next step:** a **POC** — see [poc-brief.md](poc-brief.md). **Built (3rd pass):** [demo/poc.html](../demo/poc.html). Iteration history (all red-penned, kept here so the reasoning isn't lost): (1) Penultima/meteor — thin two-option solve, tired scenario, tool-mode/modal input that could trap you; (2) *BOGWILD* (early-access) — fixed the input (left-click interact, right-click look, Spanner as an inventory item) but the gameplay was still "simple." (3) **Current — *GRIMHOLLOW*, the "sorrow leak."** A grimdark RPG where suffering is social currency; the village soft-locked because the bug is now a **diagnosis, not a labeled toggle**. You investigate (dialogue topic-trees + inspecting hidden grief values) to rule out the loudest mourner (a fraud, red herring) and find the real source: a quiet boy whose *genuine* grief overflows the shared Weeping Well because the town's grief-plumbing has no channel for the real kind. The shallow fix (clamp the boy) re-floods — root vs. symptom; the right fix opens a genuine-grief channel via a **natural Spanner minigame** (re-plumb the conduit). Then **George** (the Fixer, renamed from AUTOPATCH — after George Boole) "normalizes" the boy to spec, flattening the one real feeling in town: the villain plant and the thesis, no speech. Bill: BUG-413 → BUG-419. Bar raised toward MI3/Deponia/Gabriel-Knight density. Candidate; awaits the red pen.
