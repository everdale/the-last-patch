...
- **The antagonist** ([the-antagonist.md](the-antagonist.md)) — "The Update" reborn as **an AI that promises to fix every bug** and is coming for the Debugger's job; cannot WONTFIX. Contemporary, mirrors the player, lands the production thesis (AI without taste; the human edge is knowing when *not* to fix). Win-condition: a synthesis of Håkan's three (duel/flood it · it bankrupts itself on token costs · it lacks WONTFIX) — sufficient plan to build toward; pick the pure flavour later.
