...
- **Next step:** a **POC** — see [poc-brief.md](poc-brief.md). **First pass built:** [demo/poc.html](../demo/poc.html) — the full core loop on the BUG-031 slice (enter → Lens-inspect → patch the meteor's `targets` → bill opens BUG-047 → AUTOPATCH cameo), with the bonk kept live as tier-8. Candidate, like everything else; awaits the red pen.
